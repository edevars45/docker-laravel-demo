...
## Installation (pour suivre le tutoriel)
...
## Installation pour tester les sources

```bash
# 1) Copier les fichiers sources dans un répertoire de votre choix

# 2) Se rendre dans ce répertoire
cd chemin/vers/rpertoire/miniblog

# 3) Créer le fichier .env
cp .env.example .env

# 4) Créer une base de données MySQL ou SQLite et modifier le fichier .env
# Ex:
DB_CONNECTION=mysql
DB_HOST=127.0.0.1
DB_PORT=3306
...
# 6) Installation des dépendances Node
npm install

# 7) Faire un build des ressources front
npm run build
...
- `database/seeders/UsersSeeder.php` crée :
    ...
    - Plusieurs article pour chaque utilisateur.
